#!/usr/bin/env python3
"""
Maximum Sniper Bot — paper trading mode.

Catches pump.fun tokens at CREATION (0 seconds).
Two-phase filtering, trailing-stop exits, real bonding-curve prices.
"""
...
class Sniper:
    ...
    def _simulate_buy(
        ...
        sol_in = sol_amount * (1 - PUMPFUN_FEE_PCT) * (1 - BUY_SLIPPAGE_PCT)
        k = v_sol * v_tokens
    ...
    def _simulate_sell(
        ...
        return gross_sol * (1 - PUMPFUN_FEE_PCT) * (1 - SELL_SLIPPAGE_PCT)
    ...
    def _phase2_check(self, pos: Position) -> bool:
        return len(pos.unique_buyers) >= PHASE2_MIN_BUYERS and not pos.dev_sold
    ...
    def _close_position(self, pos: Position, reason: str, pnl_pct: float) -> None:
        ...
        log.info(
            "%s %s | %s | PnL: %+.2f%% ($%+.4f) | peak=%+.0f%% | hold=%ds | buyers=%d | bal=$%.2f",
            tag,
            pos.symbol[:10],
            reason,
            pnl_pct,
    ...
    def _open_position(self, msg: dict) -> None:
        mint = msg.get("mint", "")
        ...
        log.info(
            "BUY  %s | dev_buy=%.3f SOL | v_sol=%.1f | $%.2f bet | open=%d",
            pos.symbol[:10],
        ...
        while self._running:
            try:
                async with websockets.connect(
                    ...
                    log.info("Subscribed to newToken + tokenTrade events")
    ...
    async def _position_checker(self) -> None:
        while self._running:
            ...
            for pos in list(self.positions.values()):
                ...
                if pos.phase == "pending" and age >= PHASE2_WAIT_SEC:
                    if self._phase2_check(pos):
                        pos.phase = "confirmed"
                        self.tokens_passed_p2 += 1
                        log.info(
                            "CONF %s | buyers=%d | pnl=%+.1f%% | holding",
                            pos.symbol[:10],
                            len(pos.unique_buyers),
                            pnl,
                        )
                        ...
                        to_close.append(
                            (pos, f"P2_FAIL(buyers={len(pos.unique_buyers)},dev_sold={pos.dev_sold})", pnl)
                        )

                elif pos.phase == "confirmed":
                    reason = self._check_exit(pos, pnl)
                    if reason:
    ...
    async def _stats_printer(self) -> None:
        while self._running:
            ...
            pending = sum(1 for p in self.positions.values() if p.phase == "pending")
            confirmed = sum(1 for p in self.positions.values() if p.phase == "confirmed")
            wr = (self.total_wins / self.total_trades * 100) if self.total_trades > 0 else 0.0
            per_day = (self.total_pnl / hrs * 24) if hrs > 0 else 0.0

            log.info(
                "=== STATS %.1fh | bal=$%.2f | pnl=$%+.2f | trades=%d | WR=%.1f%% | "
                "$/day=$%+.1f | open=%d(P=%d,C=%d) | seen=%d | p1=%d | p2=%d | bl=%d ===",
                hrs,
                self.paper_balance,
                self.total_pnl,
                self.total_trades,
    ...
    async def _memory_cleaner(self) -> None:
        while self._running:
            ...
            if stale_mints or stale_curves:
                log.info(
                    "Cleaned %d closed positions, %d stale curves",
                    len(stale_mints),
                    len(stale_curves),
                )
    ...
    async def run(self) -> None:
        ...
        log.info(
            "Phase1: dev_buy=%.2f-%.1f SOL | Phase2: %ds, %d+ buyers | "
