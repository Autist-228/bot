    ...
    while True:
        ...
        for mint, token in list(tokens.items()):
            ...
            if label in ("ROCKET", "winner"):
                if checkpoint_hit not in (60, 120, 240):
                    continue
                ...
                if confidence < MIN_CONFIDENCE_PCT:
                    ...
                    continue
                cur_ratio = token.get("buy_sell_ratio", 0)
                if cur_ratio < MIN_BUY_RATIO:
                    log.info(
                        "SKIP %s %s: ratio=%.1f < %.1f",
                        label, token["symbol"], cur_ratio, MIN_BUY_RATIO,
                    )
                    continue
