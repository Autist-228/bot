...
def extract_entry_features(token, entry_ts, checkpoint_sec=60):
    ...
    sniper_sol = sum(buyer_amounts.get(addr, 0) for addr in snipers)
    early_buyer_sol_pct = sniper_sol / max(0.01, buy_sol) * 100

    mid_ts = created_ts + entry_age / 2
    buys_first_half = sum(1 for t in buys if t["ts"] <= mid_ts)
    buys_second_half = n_buys - buys_first_half
    buy_acceleration = buys_second_half / max(1, buys_first_half)

    first_sell_ts = sells[0]["ts"] if sells else entry_ts
    sell_delay_norm = (first_sell_ts - created_ts) / max(1, entry_age)

    price_vals = [t.get("price_sol", 0) for t in pre_entry if t.get("price_sol", 0) > 0]
    if len(price_vals) >= 2:
        p_mean = np.mean(price_vals)
        p_std = np.std(price_vals)
        price_volatility = p_std / max(1e-12, p_mean)
    else:
        price_volatility = 0.0

    max_streak = 0
    cur_streak = 0
    for t in sorted(pre_entry, key=lambda x: x["ts"]):
        if t["type"] == "buy":
            cur_streak += 1
            ...
            cur_streak = 0
    consecutive_buys_max = float(max_streak)

    if len(buyer_amounts) >= 2:
        amounts = list(buyer_amounts.values())
        ba_mean = np.mean(amounts)
        ba_std = np.std(amounts)
        buy_size_cv = ba_std / max(1e-12, ba_mean)
    else:
        buy_size_cv = 0.0

    net_flow_rate = net_sol_in / max(1, age)

    peak_price = max(price_vals) if price_vals else 0
    price_drawdown = (peak_price - cur_price) / max(1e-12, peak_price) * 100 if peak_price > 0 else 0.0
    ...
    return {
        ...
        "log_net_sol_flow": float(np.sign(net_sol_in) * np.log1p(abs(net_sol_in))),
        "large_buy_count": large_buy_count,
        "token_age_norm": min(entry_age / 1800.0, 1.0),
        ...
        "early_buyer_sol_pct": min(early_buyer_sol_pct, 100.0),
        "buy_acceleration": min(buy_acceleration, 10.0),
        "sell_delay_norm": min(sell_delay_norm, 1.0),
        "price_volatility": min(price_volatility, 10.0),
        "consecutive_buys_max": consecutive_buys_max,
        "buy_size_cv": min(buy_size_cv, 10.0),
        "net_flow_rate": max(-100.0, min(100.0, net_flow_rate)),
        "price_drawdown": max(0.0, min(100.0, price_drawdown)),
    }
