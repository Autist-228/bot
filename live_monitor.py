...
def check_exit_rules(sig: dict, current_pnl: float) -> str | None:
    ...
    if peak >= 15.0 and current_pnl < 0:
        return f"PROFIT_GONE (peak={peak:+.1f}%, now={current_pnl:+.1f}%)"
                    ...
                    if mint in trade_counts:
                        ...
                        for sig in signals:
                            if sig["mint"] == mint and sig["status"] == "ACTIVE":
                                initial_mcap = sig["initial_mcap_usd"]
                                if new_mcap > 0 and initial_mcap > 0:
                                    mcap_usd = new_mcap * SOL_PRICE_USD
                                    ws_pnl = ((mcap_usd - initial_mcap) / initial_mcap) * 100
                                    sig["pnl_pct"] = round(ws_pnl, 1)
                                    ...
                                    if reason:
                                        ...
                                        remaining = sig["position_remaining_pct"] / 100.0
                                        unrealized = remaining * (ws_pnl / 100.0)
                                        total = sig["realized_pnl"] + unrealized
                                        sig["pnl_usd_per_dollar"] = round(1 + total, 4)
